...
// Clase para procesar archivos PDF locales
export class LocalFileProcessor {
  // Extraer texto de un PDF
  static async extractTextFromPDF(file, setUploadProgress) {
  ...
  static async processAllFiles(files, setUploadProgress) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        // Si ya tenemos los datos procesados, usamos esos
        ...
        const { text, numPages, info } = await this.extractTextFromPDF(file, setUploadProgress);
  ...
  // Función simple para detectar idioma
